...
if __name__ == '__main__':
    ...
    filter = tf.Variable(tf.ones(shape=[kernel_h, kernel_w, 1, 1]),dtype=tf.float32)
    ...
    import numpy as np
    # image = tf.placeholder(shape=[1,1,None,None],dtype=tf.float32)
    image = tf.constant([1. for i in range(75)], shape=[1, 1, 100, 100])
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.Session() as sess:
            sess.run(init)
            res = sess.run(result,feed_dict={image:np.ones((1,1,100,100),dtype=np.float32)})
            print(res)
